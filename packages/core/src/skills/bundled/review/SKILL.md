...
- **PR number or URL** (e.g., `123` or `https://github.com/.../pull/123`):
  - Save the current branch name. Check if the working tree is dirty (`git status --porcelain`); if so, stash changes (`git stash --include-untracked`) and record that a stash was created. Then run `gh pr checkout <number>`. If checkout fails (PR doesn't exist, auth error, network issue), inform the user with the error and stop — do not proceed to review agents.
  ...
  - **Incremental review**: If `.qwen/review-cache/pr-<number>.json` exists, read the cached `lastCommitSha`. Get the current HEAD SHA. If they differ, compute the incremental diff (`git diff <lastCommitSha>..HEAD`) and use that as the review scope instead of the full diff. If the diff command fails (e.g., the cached commit was rebased away), fall back to the full diff and log a warning. If the SHAs are the same, inform the user "No new changes since last review" and stop.
