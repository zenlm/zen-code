...
export class ModelsConfig {
  ...
  async setModel(
    ...
    // Support hot-update for both qwen3.5-plus (vision-capable) and coder-model
    if (
      this.currentAuthType === AuthType.QWEN_OAUTH &&
      (newModel === 'qwen3.5-plus' || newModel === 'coder-model')
    ) {
   ...
   *
   * Examples:
   * - Qwen OAuth: coder-model -> qwen3.5-plus (same authType, hot-update safe)
  ...
  private checkRequiresRefresh(previousModelId: string): boolean {
    ...
    // (coder-model <-> qwen3.5-plus don't require ContentGenerator recreation)
    if (authType === AuthType.QWEN_OAUTH) {
      return false;
    }
